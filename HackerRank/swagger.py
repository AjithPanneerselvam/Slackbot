...
class ApiClient:
    ...
    def __init__(self, api_server='http://api.hackerrank.com'):
        #if apiKey == None:
    ...
    def callAPI(self, resourcePath, method, queryParams, postData,
        ...
        # user agent
        headers['User-Agent'] = 'Swagger/Python/0.0.5/beta'
    ...
    def sanitizeForSerialization(self, obj):
        ...
        if isinstance(obj, type(None)):
            return None
        elif isinstance(obj, (str, int, long, float, bool)):
            return obj
            ...
            return {
                self.__sanitizeKey(key): self.sanitizeForSerialization(val)
                for (key, val) in objDict.iteritems()
                if key != 'swaggerTypes' and val is not None
            }

    def __sanitizeKey(self, key):
        """
        Serialize dict key to josn key

        if key is start with '_', e.g. '_return', it becomes '_return_'
        if key is a reserved keyword, e.g. 'return', it becomes '_return'
        """
